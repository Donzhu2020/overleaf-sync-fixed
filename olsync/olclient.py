...
class OverleafClient(object):
    ...
    def all_projects(self):
        ...
        # Try original meta tag
        meta_projects = soup.find('meta', {'name': 'ol-projects'})
        if meta_projects:
            json_content = json.loads(meta_projects.get('content'))
        else:
            # Try searching for project data in HTML text using robust bracket counting
            import html
            import json
            
            # Find the start of the projects array
            text = projects_page.text
            ...
            if start_pos != -1:
                # Extract starting from the opening bracket '['
                start_index = start_pos + len(start_marker) - 1
                bracket_count = 0
                end_index = -1
                
                # Scan characters to find the matching closing bracket
                ...
                if end_index != -1:
                    raw_json = text[start_index:end_index]
                    json_content = json.loads(html.unescape(raw_json))
                else:
                    raise AttributeError("Found start of projects but could not find matching closing bracket.")
            else:
                # Fallback to preloadedProjects check
                import re
                script_content = soup.find('script', string=re.compile(r'preloadedProjects'))
                if script_content:
                    match = re.search(r'preloadedProjects\s*=\s*(\[.*?\]);', script_content.string)
                    if match:
                        json_content = json.loads(match.group(1))
                    else:
                        raise AttributeError("Could not find preloadedProjects in script tag.")
                else:
                    if "Log In" in text or "Sign Up" in text or "login" in projects_page.url:
                        raise PermissionError("Authentication failed. Your cookie might be expired.")
                    raise AttributeError("Could not find project list (ol-projects, projects array or preloadedProjects).")
